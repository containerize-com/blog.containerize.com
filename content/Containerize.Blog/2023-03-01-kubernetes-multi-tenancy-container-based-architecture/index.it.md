...
## Host container multi-tenant {#Multi-Tenant-container-hosts}
...
Utilizzerai Docker per creare l'immagine che è un modello per costruire il contenitore. Inoltre, utilizzerai Docker Composi per girare più host del contenitore. Inoltre, utilizzerai NGINX per reindirizzare le richieste in arrivo all'inquilino adeguato. Pertanto, ogni host container contiene le istanze di applicazione e database in esecuzione e in esecuzione. Questo metodo **Container Multi-Tenant** non è ad alta intensità di risorse e offre robustezza nella gestione di vari inquilini e assicurandosi l'isolamento e la sicurezza dei dati.

## Multi-tenancy di Kubernetes {#Kubernetes-Multi-Tenancy-}
