...
## 2 단계 : phpmyadmin을 설치하십시오 {#id-1-install-phpmyadmin}
...
```
**y **를 누르고** 계속하라는 요청을 받으면 **입력하십시오. 아래의** nginx **에 대한 옵션이 없으므로 웹 서버를 선택하라는 메시지가 표시되면** 탭**을 눌러 확인을 선택한 다음** 를 입력하여 웹 서버를 선택하지 않고 계속하십시오.

{{< figure align=center src="images/mysql-setup.png" alt="Ubuntu 20.04에 Nginx를 위해 Phpmyadmin을 설치하고 보안하십시오">}}
...
## 4 단계 : 기호 링크를 만듭니다 {#id-2-create-symbolic-link}
...
## 6 단계 : MySQL 슈퍼 유저를 만듭니다 {#id-4-create-mysql-superuser}
...
{{< figure align=center src="images/image.png" alt="우분투에 phpmyadmin을 로그인하는 방법 20.04">}}

phpmyadmin nginx를 확보하기 위해 phpmyadmin에 대한 추가 보안을 설정하는 것이 좋습니다. Phpmyadmin URL을 Absperure URL과 같은 것으로 변경하고 액세스 할 수 있어야합니다.

## 7 단계 : Phpmyadmin을 보호합니다 {#id-6-secure-phpmyadmin-recommended}
...
```
이제 exames.com/aspose_hidden을 방문 할 때는 인증 창이 표시되어야합니다.
...
아래에 더 관련된 기사를 좋아할 수도 있습니다.
  ...
  * [우분투의 Apache를 사용하여 OwnCloud를 설치하고 구성하는 방법][8]
