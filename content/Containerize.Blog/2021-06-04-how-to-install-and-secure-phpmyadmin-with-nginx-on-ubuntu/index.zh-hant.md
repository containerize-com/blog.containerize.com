...
```
按 **y**，然後 **輸入** ，當要求繼續。如果提示您選擇Web服務器，因為沒有選擇 **nginx** 的選項，請按 **tab** 選擇“確定”，然後**輸入** 在不選擇Web服務器的情況下繼續**。
...
## 步驟6：創建MySQL超級用戶 {#id-4-create-mysql-superuser}

在終端中，首先使用您的MySQL root密碼登錄MySQL，當您第一次安裝PhpMyAdmin MySQL數據庫時，您可能已經創建了一個root密碼。
```
sudo mysql -u root -p
```
...
## 步驟7：安全phpmyadmin {#id-6-secure-phpmyadmin-recommended}
