...
description: "이 기사에서는 2 단계 (2FA) 인증을 자세히 설명하고 IdentityServer4 및 Twilio를 사용하여 .NET5에서 구현할 수있는 방법을 설명합니다." 
...
다음 섹션이 포함되어 있습니다.
  ...
  * [2FA 사용 단점][4]
  * [.NET5에서 2FA 구현][5]
우선, 2FA가 무엇인지, 왜 모든 최신 웹 응용 프로그램에서 필수적인 부분이어야하는지 이해해 봅시다.
...
2FA에서 비밀번호는 여전히 첫 번째 인증 요소입니다. 따라서 계정에 로그인하면 계정 소유권을 확인 해야하는 다른 페이지로 리디렉션됩니다. 이것은 다양한 방법을 사용하여 수행 할 수 있습니다.
  ...
  3. 로그인 요청을 승인 할 수있는 모바일에 인증기 앱을 설치합니다.
참고 :이 검증 코드는 무작위로 생성되며 일단 사용되면 만료됩니다. 또한 수명이 짧으므로 코드를 사용하기 전에 짧은 창이 있습니다 (해커가 검증 코드를 무차별하게하는 것을 방지합니다).

## 2FA를 다단계 인증으로 부를 수 있습니까? {#MFA}
...
## 2FA 인증을 어떻게 구현할 수 있습니까? {#implementing2fa}
...
이 템플릿은 Memory Data Store를 사용하여 config.cs 파일에 테스트 사용자를 추가 할 수 있습니다.
...
이 구성 클래스는 종속성 주입을 사용하여 설정을 주입하는 데 사용됩니다.
  1. 프로젝트를 마우스 오른쪽 클릭하고 Twilio의 구성으로 사용자 비밀을 추가하십시오 (SID, Token, PhoneNumber)
