...
## この記事では、IdentityServer4やSMSプロバイダー（TWILIO）などのIDフレームワークを使用して.NET 5で2要素認証を詳細に説明します。
{{_LINE_11_}}
次のセクションが含まれています。
  * [2要素認証とは何ですか？ ][1]
  * [2FAはどのように機能しますか？][2]
...
2FAでは、パスワードは依然として最初の認証係数です。そのため、アカウントにログインすると、アカウントの所有権を確認する必要がある別のページにリダイレクトされます。これは、さまざまな方法を使用して実行できます。
  1.アプリケーションは、電子メールアドレスにOTP（1回限りのパスワード）と呼ばれることがよくある検証コードを送信します。
...
## 2FA認証を実装するにはどうすればよいですか？ {#implementing2fa}
...
注：Twilioは、電話番号を購入するために使用するすべての試用口座に15ドルのクレジットを提供します。
...
それが正しくインストールされた場合 - あなたはあなたのプロジェクトにクイックスタートフォルダーが表示されます：
...
configservicesメソッドに次のIdentityServer4サービスを追加して、startup.csを開き、次のように追加します。
```
...
```
ログインメソッドを変更して、ユーザーが2FAを有効にしたかどうかを確認して、コード検証のためにリダイレクトできます。
