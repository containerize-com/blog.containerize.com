...
## 概述
提高軟件項目生產率的最佳方法之一是自動化任何手動或重複性。當您使用像 **github Action**（例如**github Actions** ）時，您可能會驚訝於自動化的容易自動化。自從最後幾篇文章以來，我一直在寫有關[git][1]，[版本控制][1]和[軟件開發工作流程的自動化][2]和[連續集成和連續交付][3][3 ]。因此，希望到目前為止，您非常了解存儲庫以及如何使用它們。
...
**連續部署**是您的CI/CD工作流程中的一步。它不僅每次將代碼更改推向代碼庫時構建並測試您的應用程序，而且還將該應用程序連續部署到服務器。

## 什麼是github動作 {#github-actions}
...
* **工作流:** 由於GitHub操作提供了構建端到端**連續集成和連續部署的設施** 功能，因此該術語是指何時通過CI工具開始執行流程。
...
在此GitHub操作教程中，您將添加一個工作流，該工作流使用[GitHub Super-L-Lint][9]操作來凸顯多個編碼語言。每當將新提交推到您的存儲庫時，工作流程使用超級限制來驗證您的源代碼。
