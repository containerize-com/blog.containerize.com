...
## JavaScriptでAsync/async/async/async/async/async/ Asyncは構文を待っています {#What-is-AsyncAwait-in-JavaScript-Async-Await-Syntax}
...
## いつasync/asyncを使用するのですか？ {#When-to-use-AsyncAwait}
...
## async/awaitでのエラー処理 {#Error-handling-with-AsyncAwait}

エラー処理は、エンタープライズレベルのソフトウェア開発に関して重要なタスクです。 Async/awaitアプローチでエラーをキャッチするには、複数の方法があります。

{{< figure align=center src="images/async-await.png" alt="async async await javascriptをキャッチしてみてください">}}
...
* 両方のアプローチでエラー処理はかなり似ています。
 * Async待ち行列では、デバッグはそれほど難しくありません。
