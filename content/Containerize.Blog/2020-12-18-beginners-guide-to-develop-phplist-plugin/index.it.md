...
## Basics di Phlist Plugin Development   {#Basics}
...
## Crea il nostro plug -in Phplist   {#Create}
Prima di tutto, è necessario estendere la classe principale di Phplistplugin. Per creare un plug -in, è necessario creare una directory denominata "Helloworld" sotto il plugin \ _rootdir e creare un file chiamato Helloworld.php nella directory di root dei plugin. È possibile controllare il file config.php per trovare il valore per la variabile plugin \ _rootdir. Copia di seguito il codice nel file Helloworld.php.
```
<?php
