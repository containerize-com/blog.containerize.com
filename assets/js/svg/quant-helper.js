...
var RgbQuant = /** @class */ (function () {
    ...
    RgbQuant.prototype.sample = function (img, width) {
        if (this.palLocked)
    ...
    RgbQuant.prototype.reduce = function (img, retType, dithKern, dithSerp) {
        if (!this.palLocked)
            this.buildPal(undefined);
        ...
        if (dithKern)
            ...
            var data = getImageData(img, undefined), buf32 = data.buf32, len = buf32.length, out32 = new Uint32Array(len);
            for (var i = 0; i < len; i++) {
                var i32 = buf32[i];
    ...
    RgbQuant.prototype.dither = function (img, kernel, serpentine) {
        ...
        var data = getImageData(img, undefined), buf32 = data.buf32, width = data.width, height = data.height, len = buf32.length;
        ...
        for (var y = 0; y < height; y++) {
            ...
            for (var x = dir == 1 ? 0 : width - 1, xend = dir == 1 ? width : 0; x !== xend; x += dir) {
                ...
                var i32x = this.nearestColor(i32), r2 = i32x & 0xff, g2 = (i32x & 0xff00) >> 8, b2 = (i32x & 0xff0000) >> 16;
                buf32[idx] =
                    (255 << 24) | // alpha
                        (b2 << 16) | // blue
                        (g2 << 8) | // green
    ...
    RgbQuant.prototype.buildPal = function (noSort) {
        if (this.palLocked ||
    ...
    RgbQuant.prototype.palette = function (tuples, noSort) {
    ...
    RgbQuant.prototype.sortPal = function () {
        var self = this;
        this.idxi32.sort(function (a, b) {
            ...
            var hueA = rgbA[0] == rgbA[1] && rgbA[1] == rgbA[2]
                ? -1
                : hueGroup(hslA.h, self.hueGroups);
            var hueB = rgbB[0] == rgbB[1] && rgbB[1] == rgbB[2]
                ? -1
    ...
    RgbQuant.prototype.nearestColor = function (i32) {
    ...
    RgbQuant.prototype.nearestIndex = function (i32) {
        ...
        var min = 1000, idx, rgb = [i32 & 0xff, (i32 & 0xff00) >> 8, (i32 & 0xff0000) >> 16], len = this.idxrgb.length;
        for (var i = 0; i < len; i++) {
            if (!this.idxrgb[i])
                continue;
    ...
    RgbQuant.prototype.cacheHistogram = function (idxi32) {
...
var HueStats = /** @class */ (function () {
    ...
    HueStats.prototype.check = function (i32) {
        if (this.groupsFull == this.numGroups + 1)
            this.check = function () {
                return;
            };
        var r = i32 & 0xff, g = (i32 & 0xff00) >> 8, b = (i32 & 0xff0000) >> 16, hg = r == g && g == b ? -1 : hueGroup(rgb2hsl(r, g, b).h, this.numGroups), gr = this.stats[hg], min = this.minCols;
        gr.num++;
        if (gr.num > min)
            return;
        if (gr.num == min)
    ...
    HueStats.prototype.inject = function (histG) {
...
var Pr = 0.2126, Pg = 0.7152, Pb = 0.0722;
...
function isArrSortStable() {
    ...
    return ('xyzvwtursopqmnklhijfgdeabc' ==
        str
            .split('')
            .sort(function (a, b) {
...
function getImageData(img, width) {
    ...
    if (imageType === 'Array' || imageType === 'CanvasPixelArray') {
        buf8 = buf8 || new Uint8Array(img);
        imageType = 'Uint8Array';
    }
    if (imageType === 'Uint8Array' || imageType === 'Uint8ClampedArray') {
        buf8 = buf8 || img;
        buf32 = new Uint32Array(buf8.buffer);
        imageType = 'Uint32Array';
    }
    if (imageType === 'Uint32Array') {
        buf32 = buf32 || img;
...
function quantize(opts, imgData) {
    var quant = new RgbQuant(opts);
    quant.sample(imgData, undefined);
    return {
        pal8: quant.palette(undefined, undefined),
        img8: quant.reduce(imgData, undefined, undefined, undefined)
    };
}
